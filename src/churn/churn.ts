...
var log :logging.Log = new logging.Log('churn');
  ...
  // before failing.  This is needed because the browser
  // releases the UDP port asynchronously after we call close() on the
  // RTCPeerConnection, so there is no way to be sure when the port will
